...
#SingleInstance, force 
		...
		hMain		 := Panel_CreateMain(width, height, style)

		hButton		 := Panel_Add(hMain,	"Button",	"OK",		"gOnCtrl 0x8000",	"Ltop Aw")
		hBottomPanel := Panel_Add(hMain,	"Panel",	"Panel1",	"h30 style=center",	"Lbottom Aywr")
  ...
  ;add control
	if Ctrl is not Integer
	{
		if Ctrl in %integrated% 
			 f_Add := "Panel_Ctrl2Panel", 	hCtrl := %f_Add%(hPanel, Ctrl, Txt, Opt)
...
Panel_Ctrl2Panel(hParent, Ctrl, Txt, Opt ) {
...
Panel_CreateMain(width, height, style) {

	Gui, +LastFound +LabelPanel_ +Resize
	hGui := WinExist()

	Gui, Show, w%width% h%height% Hide
	Gui, Margin, 0, 0		;this makes Add function behave normaly i.e. if you add control on pos X,Y it will not be X+mx and Y+my
	return Panel_New(hGui, 0, 0, width, height, "blackrect")
