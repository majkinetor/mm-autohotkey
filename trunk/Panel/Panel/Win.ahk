...
/*
 ...
 Parameters:
 			hwnd		- Handle of the window to query
...
Win_Get(hwnd, pQ="", ByRef o1="", ByRef o2="", ByRef o3="", ByRef o4="", ByRef o5="", ByRef o6="", ByRef o7="", ByRef o8="", ByRef o9="") {
	...
	loop
	{
		...
		if !IsLabel("Window_Get_" _ )
			return "Err: Invalid query parameter: " _
		Goto Window_Get_%_%
...
Win_Is(hwnd, pQ="win") {
	...
	ifEqual, fun, , return "Err: Invalid query parameter: " pQ
...
/*
 ...
 Parameters:
			x,y,w,h		- Size / position. You can omit any parameter to keep its current value.
			flags		- Set to TRUE to redraw window after moving (default)
...
Win_MoveDelta( hwnd, xd="", yd="", wd="", hd="" ) {
	Win_GetRect(hwnd, "*xywh", cx, cy, cw, ch)
	return Win_Move( hwnd, cx+xd, cy+yd, cw+wd, ch+hd)
}
...
;--------------------------------------------------------------------------------------------------------------------- 
; Group: About 
;	o Ver 0.31 by majkinetor.
;	o MSDN Reference: http://msdn.microsoft.com/en-us/library/ms632595(VS.85).aspx
