/* About:
 	o IPC library for dotNet ver 2.5 by majkinetor <miodrag.milic@gmail.com>
...
public class IPC : NativeWindow
{
    ...
    public bool Send(IntPtr Hwnd, string Text, int Port)
	{
        ...
        cd.dwData = -Port;                                          //use negative port for textual messages
        cd.cbData = Text.Length + 1;
        cd.lpData = Marshal.StringToHGlobalAnsi(Text).ToInt32();
	...
	protected override void WndProc(ref Message m)
	{
		...
		if (m.Msg==WM_COPYDATA)	{
            ...
            if (ea.Port < 0) {
                ea.Port = -ea.Port;
                ea.Text = Marshal.PtrToStringAnsi(new IntPtr(CD.lpData), CD.cbData);
	        } else {
                ea.Data = new byte[CD.cbData];
                Marshal.Copy(new IntPtr(CD.lpData), ea.Data, 0, CD.cbData);
            }        										
            Handler( this, ea );
			return;
		}
